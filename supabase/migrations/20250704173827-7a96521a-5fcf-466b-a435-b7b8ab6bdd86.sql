...
-- Create policies for user_quiz_responses
CREATE POLICY IF NOT EXISTS "Users can manage their own quiz responses" 
ON public.user_quiz_responses 
FOR ALL 
USING (auth.uid() = user_id)
WITH CHECK (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Admins can view all quiz responses" 
ON public.user_quiz_responses 
FOR SELECT 
